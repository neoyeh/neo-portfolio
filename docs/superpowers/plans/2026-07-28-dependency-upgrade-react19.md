...
- **`typescript` was added as a new devDependency in Task 6**, not originally planned by this document, and is capped below the actual latest published version (`7.0.2` at time of writing) — this is a hard peer-dependency ceiling, not just caution. `@typescript-eslint/parser@8.65.0` and `@typescript-eslint/eslint-plugin@8.65.0` both declare `peerDependencies: { typescript: '>=4.8.4 <6.1.0' }` (confirmed via `npm view @typescript-eslint/parser@8.65.0 peerDependencies`) — TypeScript 6.1+ (and therefore 7.x) is outside the range this ESLint toolchain supports at all, for the same underlying reason that an unconstrained/too-new TypeScript broke the *old* `@typescript-eslint@2` toolchain before Task 6's upgrade (root-caused during Task 4's review). Pinned at `^6.0.3` — the highest version actually within `@typescript-eslint@8.65.0`'s supported range — rather than the original, more conservative `^5.9.3`. `react-router-dom` (`7.18.2`), `webpack` (`5.109.2`), and `webpack-cli` (`7.2.2`) were also bumped to their true latest patch releases (installed via `npm install --legacy-peer-deps`, working around the same pre-existing `eslint-config-airbnb` vs `eslint@10` peer conflict noted elsewhere in this plan); `react`/`react-dom` were independently reconfirmed already at their actual latest (`19.2.8`) via `npm view`. `typescript` remains the one package in this repo that cannot reach the npm registry's literal "latest" tag without breaking the currently-installed ESLint toolchain — bumping past `6.0.x` requires either downgrading `@typescript-eslint` (regressing Task 6) or waiting for a `@typescript-eslint` release that widens its peer range, whichever comes first.
